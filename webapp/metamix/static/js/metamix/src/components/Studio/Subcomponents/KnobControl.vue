<template>
    <div class="knob-control" :style="{ height: size-5 + 'px' }" effectId='{effectId}' audioId='{audioId}'>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            valueDisplay () {
                let out = this.valueDisplayFunction(this.value);
                this.updateFunction(out, this.effectId, this.audioId, this.type, this.name);
                return out;
            },
        },
        methods: {
            updatePosition (e) {
                ...
                let out = Math.round((v - this.min) / this.stepSize) * this.stepSize + this.min;
                this.$emit('input', out);
            },
            onClick (e) {
                if (!this.disabled) {
